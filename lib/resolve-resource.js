...
function resolveResource (type, item = '', opts = {}, fallbackHandler) {
  const { trim, uniq, find } = this.lib._
  const { fs } = this.lib
  ...
  if (opts.theme) {
    const check = `${this.app.main.dir.pkg}/extend/${this.name}/extend/${ns}/${type}/_${opts.theme}`
    file = filecheck.call(this, { dir, base, exts, check })
  }
  // check main override: common
  if (!file) {
    const check = `${this.app.main.dir.pkg}/extend/${this.name}/extend/${ns}/${type}`
    file = filecheck.call(this, { dir, base, exts, check })
  }
  ...
  if (opts.theme && !file) {
    ...
    if (theme) {
      const check = `${theme.plugin.dir.pkg}/extend/${this.name}/extend/${ns}/${extension}${type}`
      file = filecheck.call(this, { dir, base, exts, check })
    }
  }
  // check real: theme specific
  if (opts.theme && !file) {
    const check = `${this.app[ns].dir.pkg}/extend/${this.name}/${extension}${type}/_${opts.theme}`
    file = filecheck.call(this, { dir, base, exts, check })
  }
  // check real: common
  if (!file) {
    const check = `${this.app[ns].dir.pkg}/extend/${this.name}/${extension}${type}`
    file = filecheck.call(this, { dir, base, exts, check })
  }
