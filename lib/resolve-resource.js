...
function resolveResource (type, item = '', opts = {}, fallbackHandler) {
  ...
  if (opts.theme) {
    const check = `${this.app.main.dir.pkg}/extend/${this.ns}/extend/${opts.theme}/${ns}/${type}/`
  ...
  if (opts.theme && !file) {
    ...
    if (theme) {
      const check = `${theme.plugin.dir.pkg}/extend/${this.ns}/extend/${opts.theme}/${ns}/${extension}${type}`
      file = filecheck.call(this, { dir, base, exts, check })
    }
  }
  ...
  if (opts.theme && !file) {
    const theme = find(this.app.waibuMpa.themes, { name: opts.theme })
    if (theme) {
      const check = `${theme.plugin.dir.pkg}/extend/${this.ns}/extend/_common/${ns}/${extension}${type}`
      file = filecheck.call(this, { dir, base, exts, check })
    }
  }
